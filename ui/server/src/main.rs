//! Production binary for `botwork-ui-server`.
//!
//! Builds the axum router, binds, serves. Exits non-zero on:
//!
//! * bind failure (BOTWORK_UI_BIND can't be opened);
...
use botwork_ui_server::build_router;
use tokio::net::TcpListener;
use tracing::{error, info};
use tracing_subscriber::EnvFilter;

const PREFIX: &str = "[ui]";
const BIN_NAME: &str = "botwork-ui-server";
...
fn bind_from_env() -> String {
    // SECURITY: ui has no in-process authentication in v0.
    ...
    // api=9400, ui=9500).
    std::env::var("BOTWORK_UI_BIND").unwrap_or_else(|_| "0.0.0.0:9500".to_string())
}
