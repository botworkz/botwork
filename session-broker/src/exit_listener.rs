...
pub async fn serve_exit_listener(state: AppState, socket_path: &str) -> Result<(), String> {
    ...
    if let Some(parent) = std::path::Path::new(socket_path).parent() {
        if !parent.as_os_str().is_empty() {
            std::fs::create_dir_all(parent).map_err(|e| {
                format!("failed to create broker socket directory {}: {e}", parent.display())
            })?;
        }
    }
    ...
    loop {
        ...
        tokio::spawn(async move {
            ...
            let service =
                service_fn(move |req| handle_exit_request(req, Arc::clone(&state)));
...
pub async fn handle_container_exit(
    ...
    let teardown_info = {
        ...
        sessions
            .iter()
            .find_map(|(mcp_session_id, transport)| {
    ...
    let staging_path =
        format!("/var/lib/botwork/tenants/{tenant_name}/staging/{staging_token}");
