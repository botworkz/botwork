#!/usr/bin/env bash
#
# Per-service container smoke test for `botwork/api:local`.
#
...
#   docker buildx build --platform linux/amd64 --load \
#     -t botwork/api:local -f api/Dockerfile .
#   bash api/smoke.sh
...
net="botwork-smoke-api-$$"
pg="botwork-smoke-postgres-$$"
api="botwork-smoke-api-svc-$$"
...
# 3. — start api detached on the network with the alias
...
# `[api] starting on 0.0.0.0:9400`; grep for it rather
# than sleeping a fixed amount so a slow scheduler doesn't
# flake the test.
ready=0
for _ in $(seq 1 30); do
  if docker logs "${api}" 2>&1 | grep -q "\[api\] starting on 0.0.0.0:9400"; then
    ready=1
    break
  fi
  sleep 1
...
if [[ "${ready}" -ne 1 ]]; then
  echo "api did not bind in time" >&2
...
set +e
docker run --rm botwork/api:local >misslog 2>&1
miss_rc=$?
set -e
cat misslog
