...
`bootstrap.yaml`) and `botwork-admin-api` (the HTTP+JSON writer that
...
* **Apply / upsert logic.** That's `botwork-bootstrap::runner` today
  and `botwork-admin-api::write` (PR3) tomorrow.
...
* **List-level rules** — duplicate-name detection, unknown-plugin
  ...
  them while traversing its yaml tree, admin-api enforces them
...
bootstrap into this crate for [RFE #106 PR2] so admin-api can consume
