...
//! 2. Validate (admin-core for plugin specs; per-handler regex /
...
use botwork_admin_core::plugin_spec::{validate_one, RawPluginEntry};
...
// control-plane, admin-core for plugin names). Pulled into a function
...
#[serde(deny_unknown_fields)]
struct PluginBody {
    /// Bare per-entry plugin spec, validated by `botwork-admin-core`.
...
/// admin-core's validator takes a `RawPluginEntry` (its yaml-shape
/// input type) with `serde_yaml::Value` for the nested fields.
/// admin-api's JSON inputs use `serde_json::Value`, so we convert
