//! HTTP handlers for the embedded admin-ui bundle.
//!
//! v0 surface: `/healthz` and `/admin/*`. See `lib.rs` for the
//! full route table + deploy posture.
//!
//! The bundle is pulled from `admin-ui/wasm/dist/` at compile time
//! via [`include_dir!`]. The path is workspace-relative *to this
//! crate's manifest*, so the build flow is:
//!
//! 1. `trunk build --release` in `admin-ui/wasm/` populates
//!    `admin-ui/wasm/dist/` with `index.html`, the wasm-bindgen JS
//!    loader, and the `.wasm` blob.
//! 2. `cargo build --release -p botwork-admin-ui-server` inlines
...
/// `admin-ui/server/`; up one level and across into `wasm/dist/`.
///
