//! `botwork-admin-ui-server` — distroless HTTP server that serves
//! the compiled `botwork-admin-ui-wasm` bundle.
//!
//! # Why a server at all
//!
...
//!    binary lets admin-ui slot in next to admin-api with no new
...
//! 2. **Hermetic image.** `include_dir!` pulls `admin-ui/wasm/dist/`
...
//!
//! # Trust posture (mirrors admin-api / config-broker)
//!
